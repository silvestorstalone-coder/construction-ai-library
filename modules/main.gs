...
function _renderResults(action,estimateResult,technologyOutput,financeResult) {
 ...
 // TODO:Интегрировать с Sheet для вывода результатов
 ...
 // Заполнить новые результаты
 var row = 1;
 resultsSheet.getRange(row,1,1,2).setValues([
 ["Metric","Value"]
 ]);
 ...
 ["Stages",estimateResult ? estimateResult.stages.length :0],
 ["Total hours",technologyOutput ? technologyOutput.totalHours.toFixed(2) :0],
 ["Workers",technologyOutput ? technologyOutput.workers :0],
 ["Labor cost",financeResult ? financeResult.laborCost.toFixed(0) :0],
 ["Sales price",financeResult ? financeResult.salesPrice.toFixed(0) :0]
 ];
